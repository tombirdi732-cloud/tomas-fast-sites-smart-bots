...
| SMS-провайдер | договор с оператором | реализация в `SmsService.send` |
...
только пишутся в базу. `SMS_STUB=false` в production обязателен — иначе
приложение не стартует, а `SmsService.send` без провайдера бросит ошибку.
