...
с оговорками из раздела «Стартовый сервер» ниже, и настройки памяти под
него надо уменьшить. Боевой минимум — 4 ГБ и 40 ГБ: на 10 ГБ диска
не помещаются ни фотографии, ни месяц бэкапов. Значения в compose
рассчитаны именно на 4 ГБ.
...
Значения памяти в `docker-compose.prod.yml` рассчитаны на **4 ГБ**.
Под другой объём поправьте по таблице — на 2 ГБ с настройками для 8 ГБ
Postgres просто не стартует:

| Параметр | 2 ГБ | 4 ГБ (по умолчанию) | 8 ГБ |
|---|---|---|---|
| `shared_buffers` | 256MB | 1GB | 2GB |
| `effective_cache_size` | 1GB | 3GB | 6GB |
| `work_mem` | 4MB | 8MB | 16MB |
| `maintenance_work_mem` | 64MB | 128MB | 256MB |
| Redis `maxmemory` | 128mb | 256mb | 512mb |
| `NODE_OPTIONS` | 512 | 1024 | 1536 |

Логи Docker ограничены 10 МБ × 3 файла на сервис — без этого журнал
однажды забивает весь диск, и первым падает Postgres.
